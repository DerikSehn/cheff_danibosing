...
export const SectionCard = ({ item, className }: { item: ModelWithImages<Section>, className?: string }) => {
    ...
    return (
        <motion.div
            ...
            className={cn("grid md:grid-cols-4 border-t-4 border-r-8 border-primary-600 justify-center relative overflow-hidden rounded-lg bg-white-200/50 hover:bg-white-50 transition-colors  dark:bg-gray-800",
                className
            )}>
                ...
                <BentoGrid className="my-4 grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {item.images?.slice(0, 4)?.map((image, index) =>
                        ...
                        >
                            ...
                            <div className="leading-none flex flex-col space-y-1 items-center font-normal text-white-600 p-2">
                                <span className="text-xl font-light text-white-900 line-clamp-1">
                                    {image.name}
                                </span>
                                {/*  <br />
            ...
            {item?.images ? <>
                <motion.div className="relative rounded-t-3xl w-full h-full bg-white-200"
                >
                    <Image
                        alt={`section${item.title}`}
                        fill
