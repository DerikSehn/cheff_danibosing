...
export function HeroCard({ service }: { service: ModelWithImage<Service> }) {
    return (
        <Card className="relative lg:absolute rounded-md group w-full  h-dvh sm:h-auto bg-primary-300 backdrop-blur aspect-[3/4] transition-all duration-500 overflow-hidden  border-none" >
            <CardHeader className=" relative z-20 text-white-100 font-sans bg-gradient-to-b from-primary-400/30 to-50% to-primary-500/30 h-full flex justify-center space-y-4 text-center">
                <CardTitle className="text-white-100 font-sans text-4xl font-bold">{service.title}</CardTitle>
                <CardDescription
                    style={{
                        textShadow: '2px 2px 10px  black'
                    }}
                    className="lg:opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-white-100 font-sans text-sm">{service.description}</CardDescription>
            </CardHeader>
