...
export default function Sections({ sections = [] }: { sections: ModelWithImages<SectionType>[] }) {
    return (!sections.length ? null
        : <Section id="sections" className="bg-primary-200 flex flex-col justify-start py-10 h-auto px-4 min-h-[150dvh]  ">
            {/* <List
            items={sections}
            enableEditor={false}
            itemsPerPage={2}
        ...
        >
            ...
            <div className={cn(" space-y-2 text-center mx-auto px-4 my-10")}>
                ...
                <small className="text-white-100 text-sm">
    ...
    return (
        ...
        <Link href={`/section/${section?.slug}`} className={cn("flex group flex-col justify-center items-center w-full relative h-auto min-h-[50%] cursor-pointer", className)}>
            ...
            <div className="relative z-10 md:pl-[24%] space-y-10">
                <h2 className="  text-3xl font-light tracking-wide sm:text-5xl lg:text-7xl max-w-screen-md text-white-100 "
                    style={{
                ...
                <div className="font-bold text-white-100 text-lg max-w-sm tracking-wider group-hover:opacity-100 opacity-0 transition-opacity">
