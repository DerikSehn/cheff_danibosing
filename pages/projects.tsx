...
const ProjectPage = ({ projects }: { projects: ModelWithImages<ProjectType>[] }) => {
    return (<>
        ...
        <Section id="projects" className="bg-transparent  min-h-[1000px] h-auto flex flex-col max-w-screen-2xl mx-auto justify-start px-12 pt-64 ">
            <div className="relative w-full grid md:grid-cols-2 gap-8 overflow-visible text-white-50 border-b border-primary-300 mb-10">
                <div>
    ...
    return (
        <Link className="relative z-50" href={`/project/${project.id}`}>
            <Card
                ...
                <CardHeader className=" absolute inset-4 z-20 bg-primary-700/80  text-white-100 font-sans   flex justify-center space-y-4 text-center  opacity-0 group-hover:opacity-100 transition-all duration-500">
                    <CardTitle className="text-white-100  text-3xl font-moglan "
                        style={{
                    ...
                    {/*  <CardDescription
                        ...
                        className="lg:opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-white-800 opacity-50 font-sans text-4xl">
                        {project.description}
                    </CardDescription> */}
                </CardHeader>
