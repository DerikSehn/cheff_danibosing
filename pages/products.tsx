...
const ProductPage = ({ categories, initialProducts }: { categories: CategoryType[], initialProducts: ModelWithImages<ProductType>[] }) => {
    ...
    return (
        <div className="bg-white-100 relative ">
            ...
            <Section className="relative bg-transparent grid md:grid-cols-4 gap-8 pt-40 overflow-visible h-full  text-white-50 border-b border-white-300 w-full max-w-screen-2xl mx-auto items-start">
                <div className="col-span-full text-white-50 border-b border-white">
                    <div>
                ...
                >
                    {links.map(({ title, items }, index) => (
                        <ul key={`${title}-${index}`} className='px-2 flex flex-col space-y-2'>
                            <h2 className="text-white-500 p-2 text-md font-montserrat font-bold tracking-wider border-b border-white-300 ">
                                {title}
                            </h2>
                            <li className='flex space-x-2'>
                                {items.map(({ href, name }, index) => (
                                    <Link prefetch={false} key={index} href={category === name ? `/products` : href} className={cn('flex w-auto hover:bg-white-200/70 p-2 rounded-md active:bg-white-200/80', category === name && 'bg-secondary-300/10')}>
                                        <div className='text-white-600 whitespace-nowrap overflow-hidden'>
                                            {name}
                                        </div>
                                    </Link>
                                ))}
                ...
                <div className="col-span-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 bg-white-100/5 bg-clip-padding backdrop-filter rounded-lg bg-opacity-0 h-auto min-h-[600px] mb-10">
                    ...
                    {products?.length ? null : <div className="col-span-full flex flex-col items-start justify-start h-full p-4">
                        <h2 className="text-white-500 text-left text-xl font-montserrat font-bold tracking-wider">
